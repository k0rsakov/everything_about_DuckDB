...
Если вы попробовали DuckDB и решили улучшить старый SQL-код или pipeline, который написан под PostgreSQL, ClickHouse или
MySQL. Переписывать всё вручную, менять диалекты, специфичные функции даты, конкатенации — это кошмар.

Но эту проблему можно решить буквально одной библиотекой SQLGlot на Python. Это не просто "_поиск и замена_" по строкам,
это полноценный SQL-парсер и транслятор.

На вход вы скармливаете ему старый SQL-запрос. Указываете, из какого диалекта переводите, к примеру `postgres`, а на
выходе просите DuckDB (`duckdb`).

На что стоит обратить внимание:

1) это не серебряная пуля на 100% случаев. Если в вашем старом коде завязана сложная логика на хранимые процедуры (
   PL/pgSQL), специфичные оконные функции или хитрые системные таблицы — магии не произойдет, руками доработать
   придётся. Но для 80-90% стандартных ETL-запросов и аналитики SQLGlot сэкономит вам недели работы.
2) SQLGlot строит AST (абстрактное синтаксическое дерево), поэтому он понимает логику запроса, а не просто меняет слова
   местами.
3) SQL-синтаксис DuckDB очень близок к PostgreSQL, поэтому в большинстве случаев трансляция будет проходить гладко, но
   всегда стоит проверять результат.

Документация:

- [SQL Introduction](https://duckdb.org/docs/lts/sql/introduction)
...
### База DuckDB как объект

Отличительным свойством DuckDB является то, что DuckDB не имеет зависимостей, легко и просто устанавливается.

И если создать базу данных DuckDB физически, что мы рассматривали в примере [02_quick_start_cli](../02_quick_start_cli)
и [03_quick_start_dbeaver](../03_quick_start_dbeaver), то нашу базу данных `*.duckdb` можно переносить физически и
передавать коллегам.

Как это можно использовать:

- Вы настроили БД для всех своих источников, скрыли секреты и отдаете аналитикам, чтобы они могли работать с данными без
  необходимости настраивать всё это самостоятельно.
- Вы можете настроить "_автоподнятие_" БД в JupyterHub для аналитики.
